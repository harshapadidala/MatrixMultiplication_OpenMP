...
echo "* * * * * * * 10000x10000 Matrix"
cal_t=$((time bin/seq data/mat_10000x10000A.txt data/mat_10000x10000B.txt)  2>&1 > /dev/null | grep real | awk '{print $2}')
echo "with sequential    $cal_t"

cal_t=$((time mpirun -np 4 bin/mpi ddata/mat_10000x10000A.txt data/mat_10000x10000B.txt)  2>&1 > /dev/null | grep real | awk '{print $2}')
echo "with mpi(4p)       $cal_t"
echo
